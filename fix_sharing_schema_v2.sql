...
DECLARE
    r RECORD;
BEGIN
    -- Drop all policies on item_shares
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'item_shares') LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || r.policyname || '" ON item_shares';
    END LOOP;
    
    -- Drop all policies on space_members
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'space_members') LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || r.policyname || '" ON space_members';
    END LOOP;
    ...
    -- Drop all policies on spaces
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'spaces') LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || r.policyname || '" ON spaces';
    END LOOP;
END $$;
