-- ============================================
-- FIX NOTIFICATION TRIGGER - BYPASS RLS
-- ============================================
-- The issue: RLS policies are blocking the trigger from inserting notifications
-- Solution: Make the trigger function bypass RLS by using SECURITY DEFINER with proper permissions

-- Drop existing trigger and function
DROP TRIGGER IF EXISTS trigger_notify_on_block_edit ON blocks;
DROP FUNCTION IF EXISTS notify_on_block_edit();

-- Create function with SECURITY DEFINER and bypass RLS
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND (
    ...
    RAISE NOTICE 'Block edit detected: block_id=%, item_id=%', NEW.id, NEW.item_id;
...
SELECT '✅ Block edit notifications enabled with RLS bypass!' as result;
